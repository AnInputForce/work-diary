...
建议通读全文后，自行决定需要实践的步骤。后文总是有更简洁更快的实践。
...
mydjango            t1                  cc6795a90a72        23 seconds ago      699MB
```
...
```shell
ChinaDreams:z-dev kangcunhua$ docker run --name web -it -p 8000:8000 -d mydjango:t1 /bin/bash 
9326b308aacd421bbe188f39e0f8099bb8bb199f723830b00fb02203eeda4ac0
...
exit
ChinaDreams:autodeploy kangcunhua$ cd ~/workspace && docker cp web:/usr/local/web/autodeploy . && cd autodeploy && rm -rf db.sqlite3 && tree
.
├── autodeploy
│   ├── __init__.py
│   ├── __init__.pyc
...
```shell
ChinaDreams:autodeploy kangcunhua$ docker run --name web -d -it -p 8000:8000 -v ~/workspace:/usr/local/web  mydjango:t1 /bin/bash 
c4327d574b38010435b98406d2ec7c73f402c864c16a2f752e2afdd5b00c167e
